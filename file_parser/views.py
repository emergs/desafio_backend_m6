...
from .models import UploadFile, FileParserModel
...
def upload_file(request):
    if request.method == "POST":
        ...
        for list_line in file_list:
            ...
            reader = FileParserModel.objects.create(tipo=tipo, data=data, valor=valor, cpf=cpf, cartao=cartao, 
            hora=horario, dono=dono, loja=loja)

            reader.save()

        operacoes = FileParserModel.objects.values("tipo", "valor", "data", "cpf", "hora", "dono", "loja")

        saldo_total = 0

        for operacao in operacoes:
            if operacao["tipo"] == "Boleto" or operacao["tipo"] == "Financiamento" or operacao["tipo"] == "Aluguel":
                saldo_total -= operacao["valor"]
            else:
                saldo_total += operacao["valor"]
        
        return render(request, 'resultados.html', context={"operacoes": operacoes, "saldo_total": saldo_total})
